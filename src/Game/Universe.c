        ...
        for(i=0;i<universe.numPlayers;i++)
        {
            ...
            if (universe.gameStats.playerStats[i].timeOfDeath)
            {
                ...
                if (universe.gameStats.playerStats[i].typeOfPlayerDeath)
                {
                    ...
                    {
                        case PLAYERKILLED_DROPPEDOUT    : fprintf(statsFile, "%s", strGetString(strStatsDeathByDropout));                  break;
                        case PLAYERKILLED_CAPTUREDSHIP  : fprintf(statsFile, "%s", strGetString(strStatsDeathByShipCapture));              break;
                        case PLAYERKILLED_LOSTMISSION   : fprintf(statsFile, "%s", strGetString(strStatsDeathByLossOfMission));            break;
                    }
                }
            }
            else
